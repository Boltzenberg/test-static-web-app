...
namespace Boltzenberg.Functions.Commands.Telegram
{
    public class RemoveItemCommand : ICommand
    {
        ...
        public RemoveItemCommand(IJsonStore<GroceryListDocument> store)
        {
            _store = store;
        }
        ...
        public async Task<CommandResult> ExecuteAsync(CommandContext context)
        {
            ...
            {
                return CommandResult.Fail("❌ Usage: /remove <item>");
            }

            var itemToRemove = context.Arg;
            context.Log.Info("Removing item: {0}", itemToRemove);
            ...
            do
            {
                ...
                result.Entity.SetItems(domainList.Apply(Array.Empty<string>(), new[] { itemToRemove }).Items);
